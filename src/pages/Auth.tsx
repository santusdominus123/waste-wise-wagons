        ...
        <div className="mt-8 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            <Button 
              type="button" 
              variant="outline" 
              className={`group relative overflow-hidden h-12 justify-start text-left border border-white/30 rounded-2xl transition-all duration-500 ${
                activeQuickLogin === 'admin' 
                  ? 'bg-white/20 text-white shadow-lg transform scale-[1.02]' 
                  : 'bg-white/10 hover:bg-white/20 text-white/90 hover:text-white hover:shadow-lg hover:-translate-y-0.5'
              } backdrop-blur-sm`}
              onClick={() => {
                ...
                setActiveQuickLogin('admin');
                setTimeout(() => setActiveQuickLogin(''), 2000);
              }}
            >
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-gradient-to-br from-red-400 to-pink-500 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  <User className="h-4 w-4 text-white" />
                </div>
                <div>
                  <p className="font-bold text-sm">Admin Access</p>
                  <p className="text-xs opacity-75">Full system control</p>
                </div>
              </div>
              {activeQuickLogin === 'admin' && (
                <div className="absolute right-4 top-1/2 transform -translate-y-1/2 h-4 w-4 bg-green-500 rounded-full animate-pulse" />
              )}
            </Button>
            ...
            <Button 
              ...
              onClick={() => {
                ...
                setActiveQuickLogin('user');
                setTimeout(() => setActiveQuickLogin(''), 2000);
              }}
            >
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-gradient-to-br from-blue-400 to-cyan-500 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  <User className="h-4 w-4 text-white" />
                </div>
                <div>
                  <p className="font-bold text-sm">User Access</p>
                  <p className="text-xs opacity-75">Standard features</p>
                </div>
              </div>
              {activeQuickLogin === 'user' && (
                <div className="absolute right-4 top-1/2 transform -translate-y-1/2 h-4 w-4 bg-green-500 rounded-full animate-pulse" />
